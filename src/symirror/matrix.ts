...
export class Matrix4 {
    // 4次元ベクトルに4次元行列を乗算して、変換を適用する
    static transform<T extends Vector>(source: Vector, matrix: Matrix, resultTo: T): T {
        const x = source[0]!
        const y = source[1]!
        const z = source[2]!
        const w = source[3]!
    ...
    // 4次元行列の積を求める
    static mul<T extends Matrix>(a: Matrix, b: Matrix, resultTo: T): T {
    ...
    // 4次元行列の積を求める (a := a * b)
    static mulBy(a: Matrix, b: Matrix) {
        // 計算途中で a の値が変わるため、先に全要素を退避
    ...
    // 4次元以下の鏡映を表す行列を作る
    static makeReflection(normal: Vector, matrix: Matrix) {
    ...
    // 3次元空間の平行移動を表す行列を作る
    static makeTranslate(d: Vector, matrix: Matrix) {
        matrix[0] = 1
        matrix[1] = 0
        matrix[2] = 0
        matrix[3] = 0
