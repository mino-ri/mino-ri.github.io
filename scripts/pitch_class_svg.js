...
export class PitchClassSvgGenerator {
    ...
    pitchClassGroup;
    colorScheme;
    constructor(previewSvg, lineGroup, gridGroup, pitchClassGroup, colorScheme) {
        ...
        for (let i = 0; i < pitches.length; i++) {
            for (let j = i + 1; j < pitches.length; j++) {
                ...
                const intervalSteps = option.quantizeEdo > 0
                    ...
                    : 0;
                const completedInterval = autoCompleteSteps.find(s => s.step === intervalSteps || s.step === (option.quantizeEdo - intervalSteps));
                if (!completedInterval && interval.pitchDistance !== 1) {
                    continue;
                }
                const color = completedInterval?.color ??
