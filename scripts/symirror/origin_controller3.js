...
export class OriginController {
    ...
    constructor(svg, originPoint, circleGroup, canvas, onOriginChange) {
        ...
        this.#evalPoints = new Array((canvas.width + 2) * (canvas.height + 2));
    ...
    #setCanvas(polyhedron) {
        ...
        const vector0 = [0, 0, 0];
        const vector1 = [0, 0, 0];
        const distances = new Array(edgeGenerators.length + additionalLengths.length);
        ...
        for (let px = 0; px < width; px++) {
            for (let py = 0; py < height; py++) {
                const x = (px + 0.5) / width * 2.25 - 1.125;
                const y = (py + 0.5) / height * 2.25 - 1.125;
                if (x * x + y * y > 1.01 || !this.#isSpecialPoint(px, py, width))
                    continue;
                let baseX = px;
                let baseY = py;
                let range = 1;
    ...
    #getColorIndex(distances, normalCount) {
        let colorIndex = 0;
