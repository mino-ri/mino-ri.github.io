import { clearChildren, createPath, createDiamond, createCircle, createLine, centerX, centerY } from "./svg_generator.js";
import { Monzo } from "./monzo.js";
import { XLengthType } from "./pitch.js";
function getPrimePitchClass(prime, isAngleLog) {
    if (isAngleLog) {
...
function getIntervalDelta(prime, power, option) {
...
function createYLimitedLine(x1, y1, x2, y2, middlePoint, stroke, strokeWidth) {
    const { dx, dy } = middlePoint;
    return createPath(`M ${x1 + centerX},${y1 + centerY} C ${x1 + dx + centerX},${y1 + dy + centerY} ${x2 - dx + centerX},${y2 - dy + centerY} ${x2 + centerX},${y2 + centerY}`, "none", stroke, strokeWidth);
}
...
export class PitchSvgGenerator {
    ...
    maxX = 50;
    maxY = 50;
    constructor(previewSvg, grid, lineGroup, pitchClassGroup, colorScheme) {
    ...
    addIntervalLine(monzo1, monzo2, option) {
        ...
        if (interval.isOnly2) {
            ...
            if (this.lineGroup.firstChild) {
                for (let child = this.lineGroup.firstChild; child !== null; child = child.nextSibling) {
