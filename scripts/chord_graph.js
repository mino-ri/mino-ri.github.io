...
function createDiamond(x, y, size, fill, stroke, strokeWidth) {
    return createPath(`M ${x + centerX},${y + centerY - size} l ${size},${size} l -${size},${size} l -${size},-${size} Z`, fill, stroke, strokeWidth);
}
...
function createOctaveArc(x1, y1, x2, y2, scale, stroke, strokeWidth) {
    ...
    return createPath(`M ${x1 + centerX},${y1 + centerY} A ${scale * 0.625} ${scale * 0.625} 0 0 0 ${x2 + centerX},${y2 + centerY}`, "none", stroke, strokeWidth);
}
function loadMonzo(ignoreSave) {
    const text = textArea.value;
    const xLengthType = radioXOctaveReduced.checked ? XLengthType.OctaveReduced :
...
window.addEventListener("load", () => {
    ...
    grid = previewSvg.getElementById("grid");
    lineGroup = previewSvg.getElementById("line_group");
    pitchClassGroup = previewSvg.getElementById("pitch_class_group");
    const uiElements = [
        textArea,
        checkboxAutoSize,
    ...
    addEventListnerById("color_main", "input", (input) => { colorScheme.noteStroke = input.value; loadMonzo(); });
    addEventListnerById("color_sub", "input", (input) => { colorScheme.gridStroke = input.value; loadMonzo(); });
    addEventListnerById("color_fill", "input", (input) => { colorScheme.noteFill = input.value; loadMonzo(); });
    addEventListnerById("color0", "input", (input) => { colorScheme.setPitchClassColor(0, input.value); loadMonzo(); });
    addEventListnerById("color1", "input", (input) => { colorScheme.setPitchClassColor(1, input.value); loadMonzo(); });
    addEventListnerById("color2", "input", (input) => { colorScheme.setPitchClassColor(2, input.value); loadMonzo(); });
    addEventListnerById("color3", "input", (input) => { colorScheme.setPitchClassColor(3, input.value); loadMonzo(); });
    addEventListnerById("color4", "input", (input) => { colorScheme.setPitchClassColor(4, input.value); loadMonzo(); });
    addEventListnerById("color5", "input", (input) => { colorScheme.setPitchClassColor(5, input.value); loadMonzo(); });
    loadFromHash();
    loadMonzo(true);
});
